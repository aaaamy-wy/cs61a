...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return complete_apply(procedure, args, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        values = Pair(target.second, expressions.second)
        env.bindings[target.first] = do_lambda_form(values,env)
        return target.first
        # END PROBLEM 10
    else:
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            first, rest = expr.first, expr.second
            if (scheme_symbolp(first) and first in SPECIAL_FORMS):
                # if not first == "define-macro":
                #     print (first)
                result = SPECIAL_FORMS[first](rest, env)
            else:
                procedure = original_scheme_eval(first, env)
                check_procedure(procedure)
                if isinstance(procedure, MacroProcedure):
                    result = optimized_eval(complete_apply(procedure, rest, env), env)
                else:
                    operands = rest.map(lambda oprand: original_scheme_eval(oprand, env))
